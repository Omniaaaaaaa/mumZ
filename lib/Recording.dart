  ...
  Future<void> _stopRecording() async {
    try {
      ...
      if (mounted && path != null) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('تم حفظ التسجيل ✅ ($_currentSentence/$_totalSentences)'),
...
class VoiceProcessingPage extends StatefulWidget {
  const VoiceProcessingPage({Key? key}) : super(key: key);
    ...
    Future.delayed(const Duration(seconds: 5), () {
      if (mounted) {
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(
            builder: (context) => const VoiceResultPage(),
          ),
        );
      }
    });
...
class VoiceResultPage extends StatelessWidget {
  const VoiceResultPage({Key? key}) : super(key: key);
